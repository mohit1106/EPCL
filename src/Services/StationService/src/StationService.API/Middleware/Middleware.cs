...
namespace StationService.API.Middleware;
...
public class GlobalExceptionMiddleware(RequestDelegate next, ILogger<GlobalExceptionMiddleware> logger)
{
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        catch (DuplicateEntityException ex)
        {
            ...
            await context.Response.WriteAsJsonAsync(new
            {
                statusCode = 409, message = ex.Message,
                correlationId = context.Request.Headers["X-Correlation-ID"].ToString(),
                timestamp = DateTimeOffset.UtcNow
            });
        }
        ...
        catch (DomainException ex)
        {
            ...
            await context.Response.WriteAsJsonAsync(new
            {
                statusCode = 422, message = ex.Message,
                correlationId = context.Request.Headers["X-Correlation-ID"].ToString(),
                timestamp = DateTimeOffset.UtcNow
            });
        }
        catch (Exception ex)
        {
            ...
            await context.Response.WriteAsJsonAsync(new
            {
                statusCode = 500, message = "An internal server error occurred.",
                correlationId = corrId, timestamp = DateTimeOffset.UtcNow
            });
        }
    }
}
