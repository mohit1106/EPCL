...
using AIAnalyticsService.API.Middleware;
using AIAnalyticsService.Application;
using AIAnalyticsService.Infrastructure;
using Serilog;
...
try
{
    ...
    builder.Host.UseSerilog((ctx, cfg) =>
    {
        cfg.ReadFrom.Configuration(ctx.Configuration)
            ...
            .Enrich.WithProperty("ServiceName", "AIAnalyticsService")
            .WriteTo.Console()
            .WriteTo.File("logs/ai-analytics-service-.log", rollingInterval: RollingInterval.Day);
        ...
        if (!string.IsNullOrEmpty(esUrl))
        {
            ...
            {
                IndexFormat = "epcl-ai-analytics-{0:yyyy.MM.dd}",
    ...
    builder.Services.AddSwaggerGen(c =>
    {
        ...
        {
            Title = "EPCL AI Analytics Service",
            Version = "v1",
            Description = "AI-powered analytics chatbot using Google Gemini for natural language data queries."
        });
    ...
    builder.Services.AddHealthChecks()
        .AddSqlServer(builder.Configuration.GetConnectionString("DefaultConnection") ??
            "Server=localhost\\SQLEXPRESS;Database=EPCL_Analytics;Trusted_Connection=True;TrustServerCertificate=True;");
    ...
    Log.Information("EPCL AI Analytics Service starting on port 5010");
    app.Run();
}
catch (Exception ex) { Log.Fatal(ex, "AI Analytics Service terminated unexpectedly"); }
