...
using FraudDetectionService.API.Middleware;
...
try
{
    ...
    builder.Host.UseSerilog((ctx, cfg) =>
    {
        cfg.ReadFrom.Configuration(ctx.Configuration)
            ...
            .Enrich.WithProperty("ServiceName", "FraudDetectionService")
            .WriteTo.Console()
            .WriteTo.File("logs/fraud-service-.log", rollingInterval: RollingInterval.Day);
        ...
        if (!string.IsNullOrEmpty(esUrl))
        {
            ...
            {
                IndexFormat = "epcl-fraud-{0:yyyy.MM.dd}",
    ...
    {
        c.SwaggerDoc("v1", new OpenApiInfo { Title = "EPCL Fraud Detection Service", Version = "v1" });
    ...
    builder.Services.AddHealthChecks()
        .AddSqlServer(builder.Configuration.GetConnectionString("DefaultConnection") ??
            "Server=localhost\\SQLEXPRESS;Database=EPCL_Fraud;Trusted_Connection=True;TrustServerCertificate=True;");
    ...
    Log.Information("EPCL Fraud Detection Service starting on {Urls}", string.Join(", ", app.Urls));
    app.Run();
}
catch (Exception ex) { Log.Fatal(ex, "Fraud Detection Service terminated unexpectedly"); }
