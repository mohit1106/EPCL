...
try
{
    ...
    builder.Host.UseSerilog((ctx, cfg) =>
    {
        ...
        if (!string.IsNullOrEmpty(esUrl))
        {
            ...
            {
                IndexFormat = "epcl-audit-{0:yyyy.MM.dd}",
    ...
    {
        c.SwaggerDoc("v1", new OpenApiInfo { Title = "EPCL Audit Service", Version = "v1" });
    ...
    builder.Services.AddHealthChecks()
        .AddSqlServer(builder.Configuration.GetConnectionString("DefaultConnection") ??
            "Server=localhost\\SQLEXPRESS;Database=EPCL_Audit;Trusted_Connection=True;TrustServerCertificate=True;");
    ...
    Log.Information("EPCL Audit Service starting on {Urls}", string.Join(", ", app.Urls));
    app.Run();
}
catch (Exception ex) { Log.Fatal(ex, "Audit Service terminated unexpectedly"); }
