...
try
{
    ...
    builder.Host.UseSerilog((ctx, cfg) =>
    {
        ...
        if (!string.IsNullOrEmpty(esUrl))
        {
            ...
            {
                IndexFormat = "epcl-notification-{0:yyyy.MM.dd}",
    ...
    {
        c.SwaggerDoc("v1", new OpenApiInfo { Title = "EPCL Notification Service", Version = "v1" });
    ...
    builder.Services.AddHealthChecks()
        .AddSqlServer(builder.Configuration.GetConnectionString("DefaultConnection") ??
            "Server=localhost\\SQLEXPRESS;Database=EPCL_Notifications;Trusted_Connection=True;TrustServerCertificate=True;");
    ...
    Log.Information("EPCL Notification Service starting on {Urls}", string.Join(", ", app.Urls));
    app.Run();
}
catch (Exception ex) { Log.Fatal(ex, "Notification Service terminated unexpectedly"); }
